...
3. **Contextual action surface:** one region with a stable heading. Default to taking balls; selection changes it to public-card, reserved-card, or deck actions. Discard, evolution, waiting, and result phases replace its contents and never leave prior-phase buttons active.
...
- **Evolution:** show selected owned Pokémon **→** available target by full names, printed permanent-discount condition and current readiness, every legal pair in a chooser, and distinct evolve/skip buttons. Choice changes update the pair/condition; revalidate against `legalEvolutions` before dispatch. Other phases still show own readiness, including missing discounts or an unavailable target. Evolution follows discard.
- **Waiting, viewer, game over:** leave supply and mat visible, replace the action surface with phase-specific explanatory text, and disable dispatch. `GameTable` continues to enforce local/online turn ownership, not just visual disabled state. Announce phase and selection changes in a live region without duplicate noisy messages.
...
`GameTable.tsx` owns the selected item (`board`, `reserved`, or `deck`), phase precedence, legal-action derivation, focus restoration, and unchanged `dispatch(Action)` calls. A focused contextual-actions component may be extracted if it makes these modes easier to verify; it receives derived game/player facts and callbacks, not its own copy of game rules. `PlayerPanel.tsx` supplies the personal and opponent resource rows from `PlayerState`; `TokenBank.tsx` separates always-visible supply from main-phase selection actions. `CardView.tsx` remains the existing physical card face; compact facts in the action surface are text, not another sprite. Scope geometry to `.solo-game` in `styles.css`, and smoke-test the shared online `GameTable` after any shared component change. Preserve `App.tsx` save/AI flow and the current online session contract.
...
3. Exercise different-color take, take-two threshold, select/buy/reserve public card, blind reserve, buy reserved card, forced discard, evolution and skip, AI progression, and phase changes without stale controls. Complete keyboard-only selection, confirmation, Escape, and focus restoration. Verify natural scrolling and action reachability just below the supported desktop size; report the minimum supported no-scroll viewport observed. Smoke-test an online seated player with stateful action dispatch and a read-only viewer through the shared `GameTable`.
