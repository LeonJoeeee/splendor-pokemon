...
Status: **draft**. This document is for the independent pre-code design challenge. Implementation starts only after the accepted blob is recorded on issue #14 and the same lane is continued. Change the status to **committed** in the implementation PR.
...
- If a save exists, submitting valid setup presents an in-page replacement confirmation summarizing that the current saved game will be replaced. “继续现有对局” and “取消” preserve the save; only an explicit “替换并开始” creates and saves the new state. Cancel or navigation away never calls `saveSoloGame`. The same protection applies to any in-game route to New. The existing save key/schema and resume behavior remain intact.
- The active solo header retains a route back to the landing page. New setup no longer sits beside the game title. The hidden online entry points and `OnlineGame` logic remain functionally intact; shared styles must not break that screen.
...
| --- | --- | --- |
| `src/App.tsx` | Landing, setup form, save replacement confirmation, solo header and return flow | `buildSoloGame`, `loadSoloGame`, `saveSoloGame`, `nextSoloTurn`, and `OnlineGame` behavior |
...
4. Inspect the desktop browser at 1280×800 and 1440×900. Keep screenshots for orchestrator inspection. Record card count/visibility, text and art legibility, action reachability after scroll, horizontal overflow, focus navigation, and console errors. Verify image fallback if local `/sprites/` files are absent; those files are Git-ignored and are absent from this assigned checkout.
5. Exercise a new game with selected name/player count/seed, one legal human action, AI progression, refresh and continue, then New with a saved game and both cancel and explicit replace routes. Use controlled browser fixtures or a deterministic game path to reach discard and evolve, and verify their controls and dispatch. Confirm rare/legendary buy, reserved buy and blind reserve remain reachable through the normal UI when legal.
...
The change is confined to solo entry and shared presentation plus the package version. If the redesign fails acceptance, revert the implementation PR before rollout; existing saved states still load because the key and schema do not change. Preserve any pre-existing server checkout experiments during rollout. No migration or production data deletion is part of this design.
