    ...
    <div className={`player-panel ${isCurrent ? 'current' : ''} ${mine ? 'is-me' : 'is-opp'} ${compact ? 'compact' : ''}`}>
      ...
      {compact ? <div className="compact-resources" aria-label="球与加成">
        {COLOR_ORDER.map((c) => <span key={c} className="compact-resource" title={`${BALL_META[c].zh}:手牌 ${player.tokens[c]}，加成 ${player.bonuses[c]}`}>
          ...
          <b>{player.tokens[c]}<small>/</small>{player.bonuses[c]}</b>
        </span>)}
        <span className="compact-resource" title={`大师球:手牌 ${player.tokens.master}`}><span><i className="combo-dot" style={{ background: BALL_META.master.hex }} />大师</span><b>{player.tokens.master}</b></span>
      </div> : <div className="combo-row">
        {COLOR_ORDER.map((c) => {
